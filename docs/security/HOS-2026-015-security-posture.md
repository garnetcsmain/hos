...
| **RESPOND (RS)** | Governance defines an expedited breach path (24h + human); a concrete revoke/rotate/notify runbook is now published — `docs/incident-responses/RUNBOOK-revoke-rotate-notify.md` (HOS-2026-015-05). | **Partial — improving** |
...
- **Detect / monitoring (SI-4)** — access-anomaly + error/uptime alerting. **The weakest CSF
  ...
  implemented.
- **Incident-response runbook** (revoke / rotate / notify) — **published**
  (`docs/incident-responses/RUNBOOK-revoke-rotate-notify.md`, HOS-2026-015-05). Detect/monitoring
  (the alerting that would *trigger* the runbook) remains the blocked item above.
