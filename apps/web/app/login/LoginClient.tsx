...
function SupabaseLogin() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!supabase) {
      setError("El inicio de sesión con correo no está configurado en este entorno.");
      return;
    }
    setBusy(true);
    setError("");
      ...
      <form onSubmit={submitReset}>
        {resetSent ? (
          ...
          <>
            ...
            <button type="submit" disabled={busy || !email} className="mt-[18px] h-[44px] w-full rounded-[8px] bg-[var(--hos-dark)] text-[14px] font-extrabold text-white transition hover:opacity-90 disabled:opacity-60">
              {busy ? "Enviando…" : "Enviar enlace"}
            </button>
          </>
        )}
        <button type="button" onClick={() => { setMode("signin"); setResetSent(false); setError(""); }} className="mt-[14px] text-[12px] font-extrabold text-[var(--hos-muted)] underline underline-offset-2 hover:text-[var(--hos-text)]">
  ...
  return (
    <form onSubmit={submit}>
      ...
      <button type="button" onClick={() => { setMode("reset"); setError(""); }} className="mt-[14px] text-[12px] font-extrabold text-[var(--hos-muted)] underline underline-offset-2 hover:text-[var(--hos-text)]">
        ¿Olvidó su contraseña?
      </button>
      <div className="mt-[16px] border-t border-[var(--hos-border)] pt-[14px] text-[12px] font-bold text-[var(--hos-muted)]">
        ¿No tiene cuenta?{" "}
        <a href="/signup" className="text-[var(--hos-blue)] underline underline-offset-2">Crear cuenta de colaborador</a>
        <p className="mt-[6px] text-[11px] leading-[15px]">
          Cualquiera puede colaborar. El panel completo de coordinación es solo por invitación.
        </p>
      </div>
    </form>
  );
  ...
  return (
    ...
    >
      <div className="rounded-[8px] border border-[#D4DED9] bg-[#F8FAF8] px-[12px] py-[10px] text-[12px] font-bold leading-[16px] text-[var(--hos-muted)]">
        El inicio de sesión con correo no está configurado en este entorno. Falta configurar
        Supabase Auth; este token es solo el acceso de emergencia.
      </div>
      <label className={`${label} mt-[14px]`}>
        Token de coordinación
        <input className={field} type="password" value={token} onChange={(e) => setToken(e.target.value)} />
      </label>
      <button type="submit" disabled={!token.trim()} className="mt-[18px] h-[44px] w-full rounded-[8px] bg-[var(--hos-dark)] text-[14px] font-extrabold text-white transition hover:opacity-90 disabled:opacity-60">
        Continuar
      </button>
    </form>
  );
...
export function LoginClient({ supabaseConfig }: { supabaseConfig: PublicSupabaseConfig }) {
  ...
  return <Card>{configured ? <SupabaseLogin /> : <TokenFallback />}</Card>;
}
