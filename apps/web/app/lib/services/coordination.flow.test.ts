...
test("create org -> site -> need; a need starts open", () => {
  const org = seedOrg("Refugio A");
...
test("bedsFree can never exceed bedsTotal", () => {
  const org = seedOrg("Refugio B");
  const site = svc.createSite({
...
test("claim then receive: honest lifecycle with attributable events", () => {
  const requester = seedOrg("Refugio C");
  const claimer = svc.createOrg({ name: "Protección Civil", kind: "government" });
  const need = svc.createNeed({
    orgId: requester.id, siteId: null, district: "Caracas", category: "food",
    quantity: 200, unit: "raciones", urgency: "normal", notes: "",
  });

  const claimed = svc.transitionNeed({ needId: need.id, action: "claim", byOrgId: claimer.id, note: "" });
  assert.equal(claimed.status, "claimed");
  assert.equal(claimed.claimedByOrgId, claimer.id);

  const received = svc.transitionNeed({ needId: need.id, action: "receive", byOrgId: null, note: "entregado" });
  assert.equal(received.status, "received");

  const types = eventsFor("need", need.id).map((e) => e.type);
  assert.ok(types.includes("need.posted"));
  assert.ok(types.includes("need.claimed"));
  assert.ok(types.includes("need.received"));
});

test("a received need is terminal and cannot be transitioned again", () => {
  const org = seedOrg("Refugio D");
  const need = svc.createNeed({
    orgId: org.id, siteId: null, district: "Caracas", category: "medical",
    quantity: 10, unit: "dosis", urgency: "high", notes: "",
  });
  svc.transitionNeed({ needId: need.id, action: "receive", byOrgId: null, note: "" });
  assert.throws(() => svc.transitionNeed({ needId: need.id, action: "cancel", byOrgId: null, note: "" }));
});

test("only an open need can be claimed", () => {
  const org = seedOrg("Refugio E");
  const need = svc.createNeed({
    orgId: org.id, siteId: null, district: "Caracas", category: "water",
    quantity: 1, unit: "L", urgency: "low", notes: "",
  });
  svc.transitionNeed({ needId: need.id, action: "cancel", byOrgId: null, note: "" });
  assert.throws(() => svc.transitionNeed({ needId: need.id, action: "claim", byOrgId: org.id, note: "" }));
});

test("coordinationView suggests a same-category, same-district offer for an open need", () => {
  const org = seedOrg("Refugio F");
  svc.createNeed({
    orgId: org.id, siteId: null, district: "Vargas", category: "hygiene",
    quantity: 100, unit: "kits", urgency: "normal", notes: "",
  });
  svc.createOffer({ orgId: org.id, district: "Vargas", category: "hygiene", quantity: 150, unit: "kits", notes: "" });

  const view = svc.coordinationView();
...
test("createNeed with an unknown org is rejected", () => {
  assert.throws(() =>
    svc.createNeed({
