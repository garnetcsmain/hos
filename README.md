...
**Coordinator sign-in.** Coordinator endpoints are gated. By default they use a shared token (`HOS_COORDINATOR_TOKEN`, fail-closed). Optionally, **invite-only Supabase sign-in** (`HOS-2026-001-08`) can be enabled by setting `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and a `HOS_COORDINATOR_EMAILS` allowlist — then coordinators log in at **`/login`** and only allowlisted emails are admitted. When those vars are unset the app behaves exactly as before (token only). Enabling it in a real deployment requires human security sign-off (Board D3); full roles + org isolation are a follow-on that needs Postgres.
