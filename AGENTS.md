...
- **Approved cryptographic algorithms only (NIST SP 800-131A).** If you add cryptography, use an approved primitive: AES-256-GCM (symmetric), SHA-256/SHA-384/SHA-512 (hash), HMAC-SHA-256 (MAC), HKDF or PBKDF2 (KDF), Ed25519 / ECDSA-P256 (signatures), and `randomUUID` / `getRandomValues` (randomness). **Banned:** MD5, SHA-1, 3DES/DES, RC4, AES-ECB, the deprecated key-less `createCipher`/`createDecipher`, and `Math.random` for any security purpose (tokens, ids, secrets, nonces). Today the only crypto in the tree is `randomUUID` (domain/ids.ts) and `timingSafeEqual` (token compare) — both approved. The banned-primitive half is enforced by `npm run check:crypto` (`tools/checks/approved-crypto.mjs`); the `Math.random`-for-security half is review-only, since a grep cannot tell a security use from a decorative one — flag it in review.
